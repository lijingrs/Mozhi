...
use crate::shared::action_notification_popup::{ActionNotificationPopupAction, ActionNotificationPopupWidgetRefExt, ActionResult};
...
live_design! {
    ...
    use crate::shared::action_notification_popup::ActionNotificationPopup;
    ...
    ICON_AGENT = dep("crate://self/resources/icons/agent.svg")
    ICON_MCP = dep("crate://self/resources/icons/mcp.svg")
    ICON_SETTINGS = dep("crate://self/resources/icons/settings.svg")
    ICON_KNOWLEDGE_BASE = dep("crate://self/resources/icons/knowledge_base.svg")
    ICON_APP_CENTER = dep("crate://self/resources/icons/app.svg")
    ...
    ApplicationPages = <RoundedShadowView> {
        ...
        moly_agent_frame = <AgentScreen> {visible: false}
        app_center_frame = <AppCenterScreen> {visible: false}
        knowledge_base_frame = <KnowledgeBaseScreen> {visible: false}
    ...
    SidebarMenu = <RoundedView> {
        ...
        moly_agent_tab = <SidebarMenuButton> {
            text: "Agents",
            draw_icon: {
                svg_file: (ICON_AGENT),
            }
        }
        app_center_tab = <SidebarMenuButton> {
            text: "Apps",
            draw_icon: {
                svg_file: (ICON_APP_CENTER),
            }
        }
        knowledge_base_tab = <SidebarMenuButton> {
            text: "KB",
            draw_icon: {
                svg_file: (ICON_KNOWLEDGE_BASE),
            }
        }
        <HorizontalFiller> {}
        ...
        ui: <Window> {
            ...
            body = {
                ...
                notice_popup = <PopupNotification> {
                    content: {
                        popup_notice_notification = <ActionNotificationPopup> {}
                    }
                }
...
impl MatchEvent for App {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions) {
        ...
        let mut navigate_to_agent = false;
        let mut navigate_to_app_center = false;
        let mut navigate_to_knowledge_base = false;
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            radio_button_set = self.ui.radio_button_set(ids!(
                ...
                sidebar_menu.moly_agent_tab,
                sidebar_menu.app_center_tab,
                sidebar_menu.knowledge_base_tab,
        ...
        if let Some(selected_tab) = radio_button_set.selected(cx, actions) {
            ...
            match selected_tab {
                ...
                1 => navigate_to_agent = true,
                2 => navigate_to_app_center = true,
                3 => navigate_to_knowledge_base = true,
                4 => navigate_to_mcp = true,
                5 => navigate_to_providers = true,
                _ => {}
            }
        ...
        for action in actions.iter() {
            ...
            if let ActionNotificationPopupAction::Success(msg) = action.cast(){
                self.notify_action_popup(cx, &msg, ActionResult::Success);
            }
            if let ActionNotificationPopupAction::Fail(msg) = action.cast(){
                self.notify_action_popup(cx, &msg, ActionResult::Failure);
            }
            ...
            if matches!(action.cast(), ActionNotificationPopupAction::CloseButtonClicked) {
        ...
        } else if navigate_to_agent {
            self.navigate_to(cx, id!(application_pages.moly_agent_frame));
        }else if navigate_to_app_center {
            self.navigate_to(cx, id!(application_pages.app_center_frame));
        }else if navigate_to_knowledge_base{
            self.navigate_to(cx, id!(application_pages.knowledge_base_frame));
...
impl App {
    ...
    fn notify_action_popup(&mut self, cx: &mut Cx, message: &str,result:ActionResult) {
        let mut popup = self
            .ui
            .action_notification_popup(id!(popup_notice_notification));
        popup.start_auto_close_timer(cx, 2.0);
        popup.set_message(cx, message,result);
    ...
    fn navigate_to(&mut self, cx: &mut Cx, id: &[LiveId]) {
        ...
        let moly_agent_id = id!(application_pages.moly_agent_frame);
        let app_center_id = id!(application_pages.app_center_frame);
        let knowledge_base_id = id!(application_pages.knowledge_base_frame);
        ...
        if id != moly_agent_id {
            self.ui.widget(moly_agent_id).set_visible(cx, false);
        }

        if id != app_center_id {
            self.ui.widget(app_center_id).set_visible(cx, false);
        }

        if id != knowledge_base_id {
            self.ui.widget(knowledge_base_id).set_visible(cx, false);
        }
