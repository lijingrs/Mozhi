use crate::data::bot_fetcher::create_agent;
...
live_design! {
    ...
    pub AddAgentModal = {{AddAgentModal}} {
        width: Fit
        height: Fit
...
impl WidgetMatchEvent for AddAgentModal {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, scope: &mut Scope) {
        ...
        if self.button(id!(close_button)).clicked(actions) {
            cx.action(AddAgentModalAction::ModalDismissed);
        }
        ...
        if self.button(id!(add_agent_button)).clicked(actions) {
            ...
            let _= create_agent(agent.clone());
            store.insert_or_update_agent(&agent);
            cx.action(AddAgentModalAction::ModalDismissed);
            self.clear_form(cx);
        }
    }
