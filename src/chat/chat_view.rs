...
impl Widget for ChatView {
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        // 直接拼接用户输入，通过memory 获取似乎更简单 获取之后更新为空 阅后即焚
        let store = scope.data.get_mut::<Store>().unwrap();
        if let Some(user_prompt) = store.user_prompt.clone(){
            self.prompt_input(id!(chat.prompt.center.text_input)).set_text(
                cx,&user_prompt);
            store.user_prompt = None;
            let mut chat_ref = self.chat(id!(chat));
            let mut chat = chat_ref.write();
            chat.handle_submit(cx);
        }
...
impl ChatView {
    ...
    fn handle_store_sync(
        ...
        if slice_start > slice_end {
            ::log::error!(
                "Invalid slice range at handle_store_sync: {:?}",
                range
            );
            return;
        }
