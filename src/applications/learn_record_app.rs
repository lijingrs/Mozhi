...
live_design! {
    ...
    pub LearnRecordApp = {{LearnRecordApp}} {
        width: Fill,
        height: Fill,
        flow: Down,
        spacing: 15,
...
impl WidgetMatchEvent for AnswerItem {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, scope: &mut Scope) {
        ...
        if self.button(id!(ai_btn)).clicked(actions) {
            ...
            // 查询名称为当前agent的BotId
            let agent_name = "data_reporter";
            for (bot,provider) in bots {
                if provider.name == agent_name{
                    store.user_prompt = Some(format!("为我生成数据报告，<answer_id>{}</answer_id>",self.answer_id));
        ...
        if self.button(id!(edit_btn)).clicked(actions) {
            cx.widget_action(
                self.widget_uid(),
                &scope.path,
                LearnRecordAppAction::EditQuestion(self.answer_id.clone())
            );
        }
        ...
        if self.button(id!(delete_btn)).clicked(actions) {
            cx.widget_action(
                self.widget_uid(),
                &scope.path,
                LearnRecordAppAction::DeleteQuestion(self.answer_id.clone())
            );
        }
    }
}
...
impl WidgetMatchEvent for LearnRecordApp {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, _scope: &mut Scope) {
        ...
        if self.button(id!(reset_btn)).clicked(actions) {
            ...
            self.fetch_learn_record_from_api();
        }
        ...
        for action in actions {
            if let LearnRecordAppAction::EditQuestion(id) = action.cast() {
                self.handle_question_action(cx, LearnRecordAppAction::EditQuestion(id));
            } else if let LearnRecordAppAction::DeleteQuestion(id) = action.cast() {
                self.handle_question_action(cx, LearnRecordAppAction::DeleteQuestion(id));
            }else if let LearnRecordAppAction::RefreshList(page_result) = action.cast(){
                self.handle_question_action(cx, LearnRecordAppAction::RefreshList(page_result));
            }
        }
    }
}

impl Widget for LearnRecordApp {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        self.view.handle_event(cx, event, scope);
        self.widget_match_event(cx, event, scope);
    }
            ...
            if let Some(mut list) = item.as_portal_list().borrow_mut() {
                ...
                while let Some(item_id) = list.next_visible_item(cx) {
                    if item_id < self.answers.len() {
                        ...
                        // 设置 AnswerItem 的 ID
                        if let Some(mut question_item) = item_view.borrow_mut::<AnswerItem>() {
                            question_item.answer_id = question.question_id.clone();
                        }
...
impl LearnRecordApp {
    ...
    pub fn handle_question_action(&mut self, cx: &mut Cx, action: LearnRecordAppAction) {
        match action {
            LearnRecordAppAction::RefreshList(page_result) => {
            ...
            LearnRecordAppAction::FilterChanged => {
                self.apply_filter(cx);
            },
            LearnRecordAppAction::EditQuestion(id) => {
                println!("编辑题目: {}", id);
                // 发送编辑事件给上级组件
                cx.widget_action(
                    self.widget_uid(),
            ...
            LearnRecordAppAction::DeleteQuestion(id) => {
                println!("删除题目: {}", id);
                // 实现删除逻辑
    ...
    pub fn refresh_by_action(&mut self, cx: &mut Cx, questions: PageResult<AnswerRecordVO>) {
        self.handle_question_action(cx, LearnRecordAppAction::RefreshList(questions));
    }

    /// 外部调用接口：通过Action方式触发筛选
    pub fn filter_by_action(&mut self, cx: &mut Cx) {
        self.handle_question_action(cx, LearnRecordAppAction::FilterChanged);
    }
    /// 获取当前分页信息
...
impl LearnRecordApp {
    ...
    fn fetch_learn_record_from_api(&self) {
